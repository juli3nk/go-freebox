...
func (device Device) RequestAuthorization(data *bytes.Buffer) (response *TokenResponse, err error) {
	api := fmt.Sprintf("%s%s/", device.Url(""), endPointLoginAuthorization)

	client := &http.Client{}

	req, err := http.NewRequest(http.MethodPost, api, data)
...
// OpenSession :
func (device Device) OpenSession(appID, appToken, challenge string) (response *OpenSessionResponse, err error) {
